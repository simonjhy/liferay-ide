 ...
 * @author Simon Jiang
 */
public class PortalJettyBundleFactory implements PortalBundleFactory
{
    ...
    {
        return new PortalJettyBundle( location );
    }
    ...
    private boolean detectBundleDir( IPath path )
    {
        ...
        if( path.append( "bin" ).toFile().exists() && path.append( "etc" ).toFile().exists() &&
            path.append( "lib" ).toFile().exists() && path.append( "webapps" ).toFile().exists() )
        {
            return true;
        }
