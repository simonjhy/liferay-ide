 ...
 * @author Simon Jiang
 */
public class PortalGlassfishBundleFactory implements PortalBundleFactory
{
    ...
    {
        return new PortalGlassfishBundle( location );
    }
    ...
    private boolean detectBundleDir( IPath path )
    {
        ...
        if( path.append( "bin" ).toFile().exists() && path.append( "config" ).toFile().exists() &&
            path.append( "lib" ).toFile().exists() && path.append( "domains" ).toFile().exists() )
        {
            return true;
        }
