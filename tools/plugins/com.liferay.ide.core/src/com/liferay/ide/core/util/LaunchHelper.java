...
public class LaunchHelper implements IDebugEventSetListener
{
    ...
    public boolean isLaunchRunning()
    {
        return this.runningLaunch != null && !this.runningLaunch.isTerminated() &&
            ( !( this.runningLaunch.getProcesses().length > 0 &&
                this.runningLaunch.getProcesses()[0].isTerminated() ) || this.runningLaunch.getProcesses() == null );
    }
        ...
        if( isLaunchSync() )
        {
            ...
            try
            {
                while( isLaunchRunning() )
                {
                    ...
                    {
                        process = launch.getProcesses().length > 0?launch.getProcesses()[0]:null;
