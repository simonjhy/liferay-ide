 ...
 * @author Simon Jiang
 */
public class BinaryProjectsImportSDKLocationValidationService extends AbstractBinaryProjectImportSDKLocationValidationService
{
    ...
    {
        op().property( BinarySDKProjectsImportOp.PROP_PLUGINS_LOCATION ).attach( this.listener );
        op().property( BinarySDKProjectsImportOp.PROP_SDK_LOCATION ).attach( this.listener );
    }

    @Override
    protected void unBindSdkLocation()
    {
        op().property( BinarySDKProjectsImportOp.PROP_PLUGINS_LOCATION ).detach( this.listener );
        op().property( BinarySDKProjectsImportOp.PROP_SDK_LOCATION ).detach( this.listener );
    }

    @Override
    protected Path getOpPluginLocation()
    {
        Value<Path> pluginsLocation = op().getPluginsLocation();

        if ( pluginsLocation != null  && !pluginsLocation.empty() )
        {
            return pluginsLocation.content( true );
        }
