...
public class RemoveDependencyVersionCommand implements UpgradeCommand {
	...
	public void updateCommonDependency(
	...
	private IStatus _removeDependencyVersion(List<IFile> buildGradleFiles) {
		try {
			...
			buildGradleFiles.parallelStream(
			).forEach(
				buildGradle -> {
					try {
						File buildGradeFile = buildGradle.getLocation(
						).toFile();
						...
						dependencies.stream(
							...
							artifact -> {
								...
								if (targetPlatformArtifacts.contains(artifact)) {
									updateCommonDependency(updater, gradleFileContents, artifact);
								}
							}
						);
						...
						GradleUtil.refreshProject(buildGradle.getProject());
					}
					catch (Exception e) {
					}
				}
