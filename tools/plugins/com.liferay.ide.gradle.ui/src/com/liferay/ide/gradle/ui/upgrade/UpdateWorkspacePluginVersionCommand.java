	...
	private IStatus _updateWorkspacePluginVersion(File workspaceSettings) {
		try {
			...
			dependencies.stream(
				...
				artifact ->
					StringUtil.equals(artifact.getArtifactId(), workspacePluginDependency.getArtifactId()) &&
					StringUtil.equals(artifact.getGroupId(), workspacePluginDependency.getGroupId())
			).filter(
